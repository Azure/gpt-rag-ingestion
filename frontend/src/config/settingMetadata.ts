 ...
 *
 * Placeholder policy (v2.4.12): placeholders must not duplicate the example
 * shown in the hint. They describe what an empty field means at runtime —
 * "Not configured" when empty disables the feature, or "Default: <value>"
 * when the backend (see api/admin.py SETTINGS) falls back to a value.
 */
...
export const SETTING_META: Record<string, SettingMetadata> = {
  ...
  CRON_RUN_BLOB_INDEX: {
    label: "Blob index",
    tooltip:
      "Cron expression that controls when the blob indexer runs. Leave empty to fall back to the built-in default.",
  ...
  CRON_RUN_BLOB_PURGE: {
    label: "Blob purge",
    tooltip:
      "Cron expression that controls when blob deletions are reconciled into the search index. Leave empty to fall back to the built-in default.",

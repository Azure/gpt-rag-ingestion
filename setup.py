...
def call_search_api(
    ...
    try:
        ...
        if response is not None:
            ...
            if status_code >= 400:
                logging.error(
                    f"Error when calling search API {method} {resource_type} {resource_name}. Code: {status_code}"
                )
                logging.error(
                    f"Error when calling search API Reason: {response.reason}"
                )
    ...
    for connection in responseJson["value"]:
        ...
        if status == "Pending":
            ...
            response = requests.put(
                requestUrl, data=requestBodyJson, headers=requestHeaders
            )
            logging.info(
                f"Approving private link service connection {connection['name']}. Code {response.status_code}. Message: {response.reason}."
            )


def approve_search_shared_private_access(
    subscription_id, resource_group, function_app_name, storage_account_name, credential
):
    """
...
def execute_setup(
    ...
    search_api_version = "2024-05-01-Preview"  # enforced, to support indexProjections and also if the version is lower than 2023-10-01-Preview it wont work with MIS authResourceId parameter.
    ...
    # New Skills values (NEED TO BE CREATED BY HAND IN AZURE PORTAL)
    ...
    logging.info("New values")
    logging.info(f"Openai service name: {azure_open_ai_service_name}")
    logging.info(f"Openai service key: {azure_open_ai_service_key}")
    logging.info(f"Openai embedding deployment: {azure_open_ai_embedding_deployment}")
    ...
    try:
        ...
        error_message = str(e)
        logging.error(
            f"Error connecting with storage account, you may need to restart the computer. Error: {error_message}"
        )
        exit(1)
        ...
        logging.error(
            f"If you are in a network isolation scenario please run the script when connected to the solution vnet."
        )
        exit(1)
    ...
    call_search_api(
        ...
        search_api_version,
        "datasources",
        f"{search_index_name}-datasource",
        "put",
        credential,
        body,
    )
    ...
    call_search_api(
        ...
        search_api_version,
        "indexers",
        f"{search_index_name}-indexer-chunk-documents",
        "put",
        credential,
        body,
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script to do the data ingestion setup for Azure Cognitive Search."
    )
    ...
    parser.add_argument(
        ...
        "--search_principal_id",
        default="none",
        help="Entra ID of the search service",
    )
    parser.add_argument(
        "-m",
        "--azure_search_use_mis",
        help="Use Search Service Managed Identity to Connect to data ingestion function",
    )
    parser.add_argument(
        "-i",
        "--enable_managed_identities",
        action="store_true",
        default=False,
        help="Use VM's managed identities for the setup",
    )
    parser.add_argument(
        "-e",
        "--enable_env_credentials",
        action="store_true",
        default=False,
        help="Use environment credentials for the setup",
    )
    args = parser.parse_args()

    # format search_use_mis to boolean
    search_use_mis = (
        args.azure_search_use_mis.lower() == "true"
        if args.azure_search_use_mis not in [None, ""]
        else False
    )
    ...
    main(
        ...
        search_principal_id=args.search_principal_id,
        azure_search_use_mis=search_use_mis,

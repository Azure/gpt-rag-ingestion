...
def delete_skillset(
    ...
    try:
        ...
        if response.status_code == 204:
            ...
            logging.warning(
                f"Unexpected status code while deleting skillset: {response.status_code}"
            )
            logging.warning(f"Response: {response.text}")

    except requests.exceptions.ConnectionError:
        logging.error(
            "Connection error while deleting skillset. Please verify your network connection."
        )
        raise
...
if __name__ == "__main__":
    search_index_name = "ragindex-test"

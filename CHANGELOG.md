...
- **ACR remote build log streaming on Windows**: `scripts/deploy.ps1` now forces UTF-8 console/Python output and uses `az acr build --no-logs` so Unicode build output does not break Azure CLI log streaming when running from the jumpbox.

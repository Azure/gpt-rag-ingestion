...
- **Configuration tab in the ingestion dashboard ([Azure/GPT-RAG#512](https://github.com/Azure/GPT-RAG/issues/512)).** A new "Configuration" tab in the ingestion dashboard lets operators view and edit a curated allowlist of 17 runtime settings (the seven `CRON_RUN_*` schedules, chunking sizes, indexer batch/concurrency limits, processing limits, multimodal toggle, and the SharePoint per-run cap), grouped into seven sections (Scheduling, Chunking, Indexing, Throughput and concurrency, Processing limits, Multimodal, SharePoint) with accessible Radix tooltips (keyboard + screen-reader, not hover-only) explaining each setting's effect and tradeoffs. The backend adds four endpoints under `api/admin.py`: `GET /api/config` is open (so non-admins can view current values) and includes a `canEdit` field so the UI knows whether to render read-only; `PUT /api/config` is admin-gated, validates every update through a typed allowlist (int range, bool coercion, and `CronTrigger.from_crontab` for cron strings — same validator as the runtime startup, no drift), and a defense-in-depth segment denylist rejects any key whose underscore-delimited tokens overlap with secret/connection terms (`SECRET`, `PASSWORD`, `CONNECTION`, `ENDPOINT`, `APIKEY`, `CLIENTID`, `TENANTID`, `URI`, `URL`, `CONNSTR`) even if a future edit accidentally widens SETTINGS, with `200`/`207`/`422` semantics that surface per-field errors instead of partial silent failures; `POST /api/config/reload` refreshes the in-process App Configuration cache; and `POST /api/config/apply` performs an honest soft restart (refresh + reschedule every known cron job via `AsyncIOScheduler.reschedule_job` + invalidate runs/files caches) and explicitly returns `note: "In-process refresh. Hard container restart is not supported by this endpoint."` so operators are not misled. When a CRON_RUN_* changes through `PUT /api/config`, the scheduler is rescheduled immediately, eliminating any need to restart for cron edits to take effect. Non-admins under enabled auth see the tab in read-only mode with all inputs disabled and a banner explaining "Admin role required to edit"; never-exposed keys (Key Vault URI, SharePoint client secret name, MCP API key, any connection string or endpoint) are absent from the allowlist and additionally trapped by the denylist.
